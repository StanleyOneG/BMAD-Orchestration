...
function parseArgs() {
    ...
    for (let i = 0; i < args.length; i++) {
        if (args[i] === "--skills-dir") {
            ...
            opts.dryRun = true;
        } else if (args[i] === "--pi-use-manifest-names") {
            opts.piUseManifestNames = true;
            ...
            console.log(
                "  --pi-use-manifest-names  Use manifest name field as output filename for Pi agents (skips FILENAME_MAP)",
            );
    ...
    if (!["claude", "pi", "both"].includes(opts.target)) {
        console.error(
            `Error: Invalid --target "${opts.target}". Must be claude, pi, or both.`,
        );
        process.exit(1);
    }
...
function getShortName(manifestName) {
    if (manifestName.startsWith("bmad-agent-"))
        return manifestName.slice("bmad-agent-".length);
    if (manifestName.startsWith("bmad-"))
        return manifestName.slice("bmad-".length);
    return manifestName;
}
...
const AGENT_INFO = {
    analyst: {
        displayName: "Mary",
        summary: "Business analysis, research, product briefs",
    },
    architect: {
        displayName: "Winston",
        summary: "Technical architecture design",
    },
    dev: { displayName: "Amelia", summary: "Story implementation with TDD" },
    pm: {
        displayName: "John",
        summary: "PRD creation/validation, epics & stories",
    },
    sm: {
        displayName: "Bob",
        summary: "Sprint planning, story creation, retrospectives",
    },
    "ux-designer": {
        displayName: "Sally",
        summary: "UX design, wireframes, diagrams",
    },
    "quick-flow-solo-dev": {
        displayName: "Barry",
        summary: "Quick spec + dev for simple tasks",
    },
    qa: { displayName: "Quinn", summary: "Test automation" },
    "tech-writer": { displayName: "Paige", summary: "Documentation" },
    tea: {
        displayName: "Murat",
        summary: "Test architecture, ATDD, CI/CD quality gates",
    },
};
...
function discoverAgentSkills(skillsDir) {
    ...
    for (const entry of entries) {
        ...
        if (!fs.existsSync(skillMdPath)) {
            console.warn(
                `  Warning: ${entry.name} has agent manifest but no SKILL.md -- skipping`,
            );
            continue;
        }
...
// ---------------------------------------------------------------------------
function generateAgentFile(agent, platform, opts) {
    const manifest = agent.manifest;
    const name = manifest.name || agent.dirName;
    const shortName = getShortName(name);

    let outputName;
    if (opts && opts.piUseManifestNames && platform === "pi") {
        outputName = manifest.name || `bmad-agent-${shortName}`;
    } else {
        outputName = FILENAME_MAP[shortName] || `bmad-${shortName}`;
    }
    ...
    const description =
        ...
        manifest.title ||
        `BMAD ${shortName} agent`;
    const descWithName = displayName
    ...
    if (platform === "claude") {
        const tools = CLAUDE_TOOL_MAP[shortName] || [
            "Read",
            "Grep",
            "Glob",
            "Bash",
        ...
        const tools =
            PI_TOOL_MAP[shortName] || "read,write,edit,bash,grep,find,ls";
        frontmatter += `tools: ${tools}\n`;
        // Add provider/model/thinking from canonical config map
        const cfg = PI_AGENT_CONFIG_MAP[shortName];
        if (cfg) {
            frontmatter += `provider: ${cfg.provider}\n`;
            frontmatter += `model: ${cfg.model}\n`;
            frontmatter += `thinking: ${cfg.thinking}\n`;
        }
    }
...
function injectDisableModelInvocation(skillsDir, dryRun) {
    ...
    for (const entry of entries) {
        ...
        if (!content.startsWith("---")) {
            console.warn(
                `  Warning: ${entry.name}/SKILL.md has no frontmatter -- skipping injection`,
            );
            continue;
        }
        ...
        if (closingDash === -1) {
            console.warn(
                `  Warning: ${entry.name}/SKILL.md has malformed frontmatter -- skipping injection`,
            );
            continue;
        }
        ...
        if (dryRun) {
            console.log(
                `  [dry-run] Would inject disable-model-invocation into: ${entry.name}/SKILL.md`,
            );
        } else {
            fs.writeFileSync(skillMdPath, newContent, "utf8");
            console.log(
                `  Injected disable-model-invocation: ${entry.name}/SKILL.md`,
            );
        }
        count++;
    }
...
function generateSnippet(processedAgents, platform) {
    ...
    snippet +=
        "This project uses the BMAD Method for AI-driven agile development.\n";
    snippet +=
        "Available agents (invoke by name or let the host tool auto-delegate):\n\n";
...
function processTarget(
    ...
    if (!fs.existsSync(skillsDir)) {
        console.warn(
            `  Warning: Skills directory not found at ${skillsDir} -- skipping ${platform} target`,
        );
    ...
    if (written > 0) {
        if (dryRun) {
            for (const agent of agents) {
                console.log(
                    `  [dry-run] Would remove agent skill source: ${agent.dirName}`,
                );
            }
        } else {
    ...
    if (platform === "pi" && written > 0) {
        ...
        if (injected > 0) {
            console.log(
                `  ${dryRun ? "Would inject" : "Injected"} disable-model-invocation into ${injected} skill(s).`,
            );
        } else {
            console.log(
                "  No skills needed injection (all already flagged or no remaining skills).",
            );
        }
    }
...
function main() {
    ...
    if (opts.target === "both" && opts.skillsDir) {
        console.warn(
            'Warning: --skills-dir is ignored when --target is "both" (each platform uses its own skills directory).',
        );
    }
    ...
    for (const processed of allProcessed) {
        if (processed.agents.length > 0) {
            ...
            const configFile =
                processed.target === "claude" ? "CLAUDE.md" : "PI.md";
            console.log("\n" + "=".repeat(60));
